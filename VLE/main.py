...
def label_enhance(enc, datas, labels, args):
    ...
    for i in range(n_batches):
        ...
        # forward
        (mu, sigma) = enc(batch_data)
        d = F.softmax(mu[:,-args['num_class']:], dim=1)
...
def main(args):
    ...
    # create model
    enc = VAE_Gauss_Decoder(n_in=train_data.n_feature + train_data.n_label, n_hidden=args['n_hidden'], n_out=args['dim_z'], keep_prob=args['keep_prob'])
    dec = VAE_Bernulli_Decoder(n_in=args['dim_z'], n_hidden=args['n_hidden'], n_out=train_data.n_feature, keep_prob=args['keep_prob'])
    optimizer = torch.optim.Adam(list(enc.parameters())+list(dec.parameters()),lr=args['learning_rate'],weight_decay=0)
    ...
    targets = load("SJAFFE", return_X_y=False).label_distribution
    print(kldivergence(targets, preds))
    # save distributions
